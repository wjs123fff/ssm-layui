...
<div id="pagenum">
        <ul class="pagination">
        <li><a href="${ctx}/notice/list?page=1&size=5" aria-label="Previous">首页</a></li>
        <li><a href="${ctx}/notice/list?page=${pageInfo.pageNum-1}&size=5">上一页</a></li>
        <c:forEach begin="1" end="${pageInfo.pages}" var="pageNumber">
            <li><a href="${ctx}/notice/list?page=${pageNumber}&size=5">${pageNumber}</a></li>
        </c:forEach>
        <li><a href="${ctx}/notice/list?page=${pageInfo.pageNum+1}&size=5">下一页</a></li>
        <li><a href="${ctx}/notice/list?page=${pageInfo.pages}&size=5" aria-label="Next">尾页</a></li>
    </ul>
...
<%--      </div>--%>
    ...
    <script>
      ...
      /*用户-删除*/
      function member_del(obj,notice_id){
          layer.confirm('确认要删除吗？',function(index){
              //发异步删除数据
              //等以后再使用异步，这里先使用
              $.get("${ctx}/notice/delete?id="+notice_id);
