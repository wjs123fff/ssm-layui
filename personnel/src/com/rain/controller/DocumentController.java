	...
	// 如果在目录下输入为空，则跳转到指定链接
		...
		@RequestMapping(value="/document/list",method=RequestMethod.GET)
		 public String index(Model model,String content, @RequestParam(defaultValue = "1") int page, @RequestParam(defaultValue = "5")int size){
			List<Document> document_list = rainservice.get_DocumentList(page,size);
			PageInfo pageInfo=new PageInfo(document_list);
			model.addAttribute("pageInfo",pageInfo);
			if (content!=null){
				document_list = rainservice.get_DocumentLikeList(content);
			}
			model.addAttribute("list",document_list);
		...
		@RequestMapping(value="/document/add",method=RequestMethod.POST)
		 public ModelAndView add(ModelAndView mv,@ModelAttribute Document document ,Integer id,HttpSession session
			...
			if(id!=null){
				...
				path = "D:\\IDEAwork\\personnel\\file";
